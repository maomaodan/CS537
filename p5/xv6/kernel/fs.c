...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  if (ip->type == T_SMALLFILE)
  {
    cprintf("read n: %d, size %d\n",n,ip->size);
    start_data = ((char*)(ip->addrs)+off);
    if(n+off>52)
      return -1;
    memmove(dst, (void*)start_data, ip->size-off);
    //cprintf("dst[0]: %d,%d\n",*dst,*start_data);
    return n;
  }
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  if (ip->type == T_SMALLFILE)
  {
    ...
    memmove((void*)(start_data),src, byteswritten);//prepare data in buffer
    //cprintf("src[0]: %d,%d\n",*src,*start_data);
    //cprintf("src[1]: %d,%d\n",*(src+1),*(start_data+1));
    
      ip->size = off + byteswritten;
      iupdate(ip);
    return byteswritten;
  }
